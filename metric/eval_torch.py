...
if __name__ == '__main__':
    if __name__ == '__main__':
        with_mean = True
        config = {
            'dataroot': r'../test_img/',  # Change to your local infrared and visible images path
            'results_root': r'../test_result/',  # Change to your local fusion images path  -fusion_result
            'dataset': r'TNO',  # Specify the dataset name
            'save_dir': r'../tno_xlsx'  # Directory for saving metrics
        }
        ...
        filelist = natsorted(os.listdir(ir_dir))[:360]
        ...
        Method_list = [
            'RFN-Nest','TarDAL','ReCoNet','CMTFusion','LRRNet','MPCFusion','FISCNet'
        ]
        # Method_list = [
        #     'Ours','SDNet','RFN-Nest','TarDAL','ReCoNet','CMTFusion','CDDFuse','IGNet','LRRNet','MPCFusion','SFDFusion',
        #     'FISCNet'
        # ]

        # Starting index for the method 'BDLFusion'
        start_index = Method_list.index('RFN-Nest')
